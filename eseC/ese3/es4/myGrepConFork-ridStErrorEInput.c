...
int main (int argc, char** argv)
{
	...
	if (N != 2)
	{
		printf("Errore nel numero di parametri che devono essere due (stringa da cercare e nome del file dove cercare), forniti %d\n", N);
		exit(1);	
	}
	...
	/* ridirezionamo lo standard output su /dev/null perche' ci interessa solo se il comando grep ha successo o meno */
        ...
        if(open(argv[2], O_RDONLY) < 0){
			printf("Errore apertura file di input %s\n", argv[2]);
			exit(-1);
		}
