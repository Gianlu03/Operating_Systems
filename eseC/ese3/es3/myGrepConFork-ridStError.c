...
int main (int argc, char** argv)
{
	...
	if (N != 2)
	{
		printf("Errore nel numero di parametri che devono essere due (stringa da cercare e nome del file dove cercare), inseriti: %d\n", N);
		exit(1);	
	}
	...
	/* ridirezionamo lo standard output su /dev/null perche' ci interessa solo se il comando grep abbia successo o meno */
		close(1);
		if(open("/dev/null", O_WRONLY) < 0){
			perror("Errore apertura /dev/null per stdout");
			exit(-1);
		}
