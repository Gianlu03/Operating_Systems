...
int main(int argc, char** argv){
    ...
    char* USAGE = "USAGE: ./main fileAssol1 fileAssol2 fileAssol3 [fileAssolN]\n"; /* Variabile con stringa da stampare di USAGE in caso di errore nei parametri*/

    Messaggio mess;

    /*Controllo numero parametri forniti*/
    if(argc < 4){
        printf("Errore: necessari almeno 3 parametri, ne sono stati inseriti %d \n", argc - 1);
        printf("%s", USAGE);
        exit(1);
    }
    int N = argc - 1; /*Variabile N coincide ad argc senza nome file*/

    printf("DEBUG: numero parametri corretto, inseriti %d\n", N);
    ...
    for(int i = 0; i < N; i++){
        ...
        if(pid == 0){
            ...
            while(read(pipeNipote[0], &(mess.c3[j]), 1)){
                if(mess.c3[j] == '\n'){ /* incontro terminatore */
                    j++;
                    break;
                }
                j++;
            ...
            write(piped[i][1], &mess, sizeof(Messaggio));
                
            exit(mess.c2 - 1); //ritorno il valore anche tramite exit alla wait
        }
    ...
    /*Padre inizia chiudendo le pipe inutilizzate*/
    for(int i = 0; i < N; i++)
        close(piped[i][1]);
    
    printf("\n");
    for(int i = 0; i < N; i++){
        /*Leggo la struct da pipe e setto il terminatore alla stringa*/
        read(piped[i][0], &mess, sizeof(Messaggio));
        mess.c3[mess.c2 - 1] = '\0';

        printf("Il nipote di PID = %d ha lavorato sul file %s:\n", mess.c1, argv[i+1]);
        printf("Linea letta:\n");
        printf("%s\n", mess.c3);
        printf("Lunghezza: %d\n\n", mess.c2);
    }
